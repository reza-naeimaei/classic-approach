...
def extract_realization_from_v73(file_path: str | Path, epoch_kind: str, realization_idx: int) -> dict:
    ...
    try:
        ...
        # Surface / fitted-point geometry and map from CP stack to fitted-point stack.
        lhat_stack = None
...
def variance_factor_compatibility(s01_sq: float, f1: int, s02_sq: float, f2: int, alpha: float = 0.05) -> dict:
    s01_sq = float(s01_sq)
    s02_sq = float(s02_sq)
    if s01_sq <= 0 or s02_sq <= 0:
        return {"compatible": False, "F": np.nan, "crit": np.nan, "alpha": alpha}
    if s01_sq >= s02_sq:
        Fval = s01_sq / s02_sq
        dfn, dfd = int(f1), int(f2)
    else:
        Fval = s02_sq / s01_sq
        dfn, dfd = int(f2), int(f1)
    crit = float(f_dist.ppf(1 - alpha / 2, dfn=dfn, dfd=dfd))
    return {"compatible": bool(Fval <= crit), "F": Fval, "crit": crit, "alpha": alpha, "dfn": dfn, "dfd": dfd}
...
def compare_under_null(removed_idx_zero_based: np.ndarray, n_cp: int) -> dict:
    pred_ids = np.sort(np.asarray(removed_idx_zero_based, dtype=int) + 1)
    n_removed = int(pred_ids.size)
    return {
        "predicted_ids_1based": pred_ids.tolist(),
        "n_removed": n_removed,
        "n_fp": n_removed,
        "n_tn": int(n_cp - n_removed),
        "local_any_false_alarm": bool(n_removed > 0),
        "local_fp_rate_over_cp": float(n_removed / n_cp),
    }


def build_surface_pointwise_type1_mask(d_cp: np.ndarray, Qdd: np.ndarray, A_map: np.ndarray, s0_pool_sq: float, alpha: float, f_total: int) -> dict:
    ...
    for j in range(n_surf):
        sl = slice(3 * j, 3 * j + 3)
        B = A_csr[sl, :].toarray()  # 3 x n_param
    ...
    return {
        ...
        "n_points": int(n_surf),
        "n_fp": int(reject_mask.sum()),
        "local_any_false_alarm": bool(reject_mask.any()),
        "local_fp_rate_over_surface": float(reject_mask.mean()),
        "T": T_vals,
        "crit": crit_vals,
    }
...
def process_realization(i: int, ref_mat_path: str, comp_mat_path: str, scenario: str):
    ...
    d = ep2["CP_stack"] - ep1["CP_stack"]
    Q_dd = ep1["Qxx"] + ep2["Qxx"]
    vf = variance_factor_compatibility(ep1["s0_sq"], ep1["dof"], ep2["s0_sq"], ep2["dof"], alpha=COMPATIBILITY_ALPHA)
    ...
    per_real_payload = {
        ...
        "variance_factor_compatibility": vf,
    ...
    for a in ALPHAS:
        g = global_congruency_test_F(d, Q_dd, s0_pool_sq, alpha=a, f_total=f_total)
        loc_cp = sequential_localization_A(d, Q_dd, s0_pool_sq, alpha=a, f_total=f_total)
        cmp_cp = compare_under_null(loc_cp["removed_idx"], n_cp=ep1["CP_xyz"].shape[0])
        surf = build_surface_pointwise_type1_mask(d, Q_dd, ep1["A_map"], s0_pool_sq, alpha=a, f_total=f_total)

        row = {
            "realization_idx_one_based": i + 1,
            "scenario": scenario,
            "alpha": a,
            "vf_compatible": bool(vf["compatible"]),
            ...
            "global_type1_error": bool(g["reject"]),
            "n_removed": int(loc_cp["removed_idx"].size),
            "n_fp": cmp_cp["n_fp"],
            "local_any_type1_error": bool(cmp_cp["local_any_false_alarm"]),
            "local_fp_rate_over_cp": float(cmp_cp["local_fp_rate_over_cp"]),
            "surface_n_fp": int(surf["n_fp"]),
            "surface_any_type1_error": bool(surf["local_any_false_alarm"]),
            "surface_fp_rate_over_points": float(surf["local_fp_rate_over_surface"]),
        }

        per_real_payload["alphas"][str(a)] = {
            "global": g,
            "cp_localization": {
                "removed_idx_zero_based": loc_cp["removed_idx"].tolist(),
                "removed_idx_one_based": (loc_cp["removed_idx"] + 1).tolist(),
                "n_removed": int(loc_cp["removed_idx"].size),
                "reject_mask": loc_cp["reject_mask"].tolist(),
            },
            "surface_pointwise": {
                "n_points": int(surf["n_points"]),
                "n_fp": int(surf["n_fp"]),
                "local_any_false_alarm": bool(surf["local_any_false_alarm"]),
                "local_fp_rate_over_surface": float(surf["local_fp_rate_over_surface"]),
                "reject_mask": surf["reject_mask"].tolist(),
            },
            "comparison_cp": cmp_cp,
        }
        rows.append(row)

    return {"i": i, "payload": per_real_payload, "rows": rows, "first_cp_xyz": ep1["CP_xyz"], "first_surface_xyz": ep1["surface_xyz"]}
    ...
    for a in ALPHAS:
        ...
        summary["alphas"][str(a)] = {
            "type1_null": {
                ...
                "global_type1_rate": mean_or_none([r.get("global_type1_error") for r in rows_a]),
                "cp_any_false_alarm_rate": mean_or_none([r.get("local_any_type1_error") for r in rows_a]),
                "cp_mean_false_alarm_count": mean_or_none([r.get("n_fp") for r in rows_a]),
                "cp_mean_false_alarm_rate": mean_or_none([r.get("local_fp_rate_over_cp") for r in rows_a]),
                "surface_any_false_alarm_rate": mean_or_none([r.get("surface_any_type1_error") for r in rows_a]),
                "surface_mean_false_alarm_count": mean_or_none([r.get("surface_n_fp") for r in rows_a]),
                "surface_mean_false_alarm_rate": mean_or_none([r.get("surface_fp_rate_over_points") for r in rows_a]),
            }
        }
    return summary
...
if __name__ == "__main__":
    ...
    comp_nodefo_mat_path = "../00_dam_syntetic/results_v2/EPcomp_MCS1000_addedbias_id1_noDefo.mat"
    results_dir = Path("./cls_results_all_type1_100_update")
    per_real_dir = results_dir / "per_realization"

    results_dir.mkdir(parents=True, exist_ok=True)
    per_real_dir.mkdir(parents=True, exist_ok=True)

    tasks = [("type1_null", i, ref_mat_path, comp_nodefo_mat_path) for i in range(n_rep)]
    ...
    # Save geometry from the first realization for later plotting.
    np.save(results_dir / "first_cp.npy", results[0]["first_cp_xyz"])
    np.save(results_dir / "first_surface_xyz.npy", results[0]["first_surface_xyz"])
    ...
    save_json(results_dir / "dam_cls_type1_rows.json", {"rows": all_rows})
    save_json(results_dir / "dam_cls_type1_summary.json", build_summary(all_rows))

    csv_path = results_dir / "dam_cls_type1_rows.csv"
